...
def stat():
    yzm_=input('验证码:')#手动输入验证码
    userid=input('学号:')
    userpassword=input('密码(如为学号可直接回车):')
    if(userpassword==''):
        userpassword=userid

    json1=login(yzm_,userid,userpassword)#登录执行
    return json1