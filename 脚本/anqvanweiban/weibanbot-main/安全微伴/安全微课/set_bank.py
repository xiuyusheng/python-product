...
def Storage(token, userId, prepare_startpaper_head,num):
    listPlan_= listPlan(userId, prepare_startpaper_head,getTask(token, userId, prepare_startpaper_head, num))
    if(not listPlan_==[]):
        getlisthistory_=getlisthistory(userId, prepare_startpaper_head,listPlan_)
                ...
                if(pdxd(question['id'])):
                    ...
                    for Answer in question['optionList']:
                        if(Answer['isCorrect']==1):
                            Answers+=Answer['id']+','
                    Answers=Answers[:-1]
                    aa=conn.execute(f"INSERT INTO '{Library}'(questionId,Answer) values('{question['id']}','{Answers}')")
                    # print(aa)
