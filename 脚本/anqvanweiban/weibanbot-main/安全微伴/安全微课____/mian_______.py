...
def listStudyTask(User):  # 获取课程信息
    ...
    listStudyTask_response = requests.post(
        url=listStudyTask_url, headers=listStudyTask_head, data=listStudyTask_data
    ).json()
...
def userName(User):
    ...
    tenantCode = login_["tenantCode"]
    index_url = (
        f"https://weiban.mycourse.cn/pharos/my/index.do?timestamp={int(time.time())}"
    )
    index_head = {
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36 Edg/107.0.1418.35",
        "x-token": token,
    ...
    listStudyTask_response = requests.post(
        url=index_url, headers=index_head, data=index_data
    ).json()
...
def start(userId, password, studyName):
    ...
    if not start.studyID == "":  # 判断是否存在该课程
        # print(start.studyID)
        print("考生：" + userName, userName, f"课程：{studyName}", sep="\n")
        start.study()
        start.examination()
    else:
...
if __name__ == "__main__":
    studyName = [
        ...
        ["2106200420", "2106200420", "2023年春季专题"],
