...
import random
import sys
from common import SchoolLogin, LoginError, get_info
...
class PostInfo(object):  # 提交函数，用于提交打卡信息
    ...
    def main(self):  # 执行函数
        try:
            # s = SchoolLogin(self)
            # Cookie_url="http://112.74.78.185:8000/school_info/"
            # resp=requests.get(Cookie_url)
            # print(resp.json())
            self.cookie = 'MOD_AUTH_CAS=IxvqrTEya1A9yB2J9bdNd31657924766'
