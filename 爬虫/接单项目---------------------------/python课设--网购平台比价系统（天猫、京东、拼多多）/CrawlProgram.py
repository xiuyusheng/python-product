...
class CrawlProgram:
    ...
    def getItemsFromgome(self, keyword, number):
        ...
        soup = requests.get(url=url, headers=head).json()
        title_ls = list()
        href_ls = list()
        id_ls = list()
        sale_ls = list()
        ...
        items = list()
        try:
            for i in soup['content']['prodInfo']['products']:
                title_ls.append(i['alt'])
                href_ls.append('https:'+i['sUrl'])
                id_ls.append('G'+i['skuId'])
                sale_ls.append('无相关信息')
                try:
                    price_ls.append(requests.get('https://ss.gome.com.cn/search/v1/price/single/{}/{}/{}/{}/31010000/flag/item'.format(i['saleOrgId'] if i['saleOrgId'] else 'null',i['shopId'],i['pId'],i['skuId']),headers=head).json()['result']['price'])
                except:
                    price_ls.append(-1)
                items.append(Goods(id_ls[-1], '国美', title_ls[-1],
                            '无相关信息', price_ls[-1], sale_ls[-1], href_ls[-1]))
            return items[:number]
        except Exception as e:
            print('获取拼多多商品信息失败{}'.format(e))
            return []
    ...
    def getItemsFromJD(self, keyword, number):
        ...
        soup = bs(self.getHTMLText(url) +
                  self.getHTMLText(url2), 'html.parser')
        try:
            # get_price
            price_ls = []
            ...
            for num in range(len(title_ls)):
                goods = Goods(id_ls[num], '京东', title_ls[num], shop_ls[num], eval(
                    price_ls[num]), '无相关信息', href_ls[num])
                items.append(goods)
            return items[:number]
        except:
            print('获取京东商品信息失败')
    ...
    def getItemsFromPinduoduo(self, keyword, number):
        ...
        try:
            for i in soup.find('ul', {'id': 'component_59'}):
                ...
                id_ls.append('P'+i['id'])
                sale_ls.append('无相关信息')
                price_ls.append(re.search(r'\s*¥(?P<price>\d+\.*\d*)(定价)*',i.find('p', {'class': 'price'}).text).group('price'))
                items.append(Goods(
                    id_ls[-1], '拼多多', title_ls[-1], '无相关信息', float(price_ls[-1]), sale_ls[-1], href_ls[-1]))
            return items[:number]
        except:
            print('获取拼多多商品信息失败')
            return []
...
if __name__ == '__main__':
    ...
    for it in its:
        print(it.getID(), it.getPlatform(), it.getTitle(),
              it.getShop(), it.getPrice(), it.getSales(), it.getHref())
