...
if __name__ == "__main__":
    ...
    for i in range(19, 21):
        ...
        head = head_get()
        resp = session.get(
            "https://piao.qunar.com/ticket/list.json?keyword=%E6%AD%A6%E6%B1%89&region=null&from=mps_search_suggest&page={}".format(
                i + 1
            ),
            headers=head,
        )
        ...
        try:
            for j in resp.json()["data"]["sightList"]:
                session.get(
                    "https://piao.qunar.com/ticket/detail_824368214.html?st={}".format(
                        j["searchTrace"]
                    ),
                    headers=head,
                )
                ...
                try:
                    zz = session.get(
                        "https://piao.qunar.com/ticket/detailLight/sightCommentList.json?sightId=201733&index=1&page=1&pageSize=10&tagType=0",
                        headers=head,
                    ).json()["data"]
                    point = j["point"].split(",")
                    ...
                    work_sheet.write(
                        num,
                        5,
                        str(int(zz["tagList"][6]["tagNum"] / zz["commentCount"] * 100))
                        + "%",
                    )
                    num += 1
