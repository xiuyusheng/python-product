...
class WB():
    def __init__(self,key_word,WK) -> None:
        ...
        self.new_data = WK['workbook']#表格实例
    ...
    def save_work(self):
        self.new_data.save('微博({}).xls'.format(self.key))
    ...
    def write_excel(self,k):
        ...
        if (not k['id'] in self.save_ids) and (2008<int(k['created_at'][-4:]) and int(k['created_at'][-4:])<2022):
            ...
            if text_:
                ...
                text_=re.search(r'^.{,10}',text).group()
        ...
        for key in self.key_word:
            ...
            for year in range(1900,2022):
                ...
                for type_ in (1,2):
                    ...
                    resp=self.session.get(url=url,headers=self.head,params=params).json()
                    # print(resp)
                    if resp['ok']:
                        self.key_=False
...
if __name__ == "__main__":
    ...
    wk=xlwt.Workbook()
    MK['workbook']=wk
    list_=[[WB('低碳',MK)],[WB('双碳',MK)],[WB('排放碳',MK)],[WB('双减',MK)],[WB('减碳',MK)],[WB('减碳排',MK)],[WB('节能减排',MK)]]
    pool = Pool(len(list_))
    pool.starmap(main, list_)
    wk.save('微博.xls')